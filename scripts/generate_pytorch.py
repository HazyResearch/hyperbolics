...
def build(run_name, epochs=100):
    os.mkdir(run_name)

    for dataset in range(1,13):
        for rank in [2,5,10,50,100,200]:
            run_stem = f"{run_name}/dataset_{dataset}.r={rank}"
            exec_str = f"CUDA_VISIBLE_DEVICES=\"1\" python pytorch/pytorch_hyperbolic.py learn {dataset} --model-save-file {run_stem}.model  -r {rank} --batch-size 1024 -g --epochs {epochs} --log-name {run_stem}.log"
            print(exec_str)
