...
datasets = os.listdir("./data/edges/parsing/ewt/ewt_dev/")
...
models10 = [
    ...
    {'dim': 0, 'hyp': 0, 'edim': 2, 'euc': 1, 'sdim': 0, 'sph': 0},
...
def run_pytorch(run_name, gpus, gpc, epochs, batch_size):
    ...
    stuff = itertools.product(datasets, models, lrs)
    hparams = list(stuff)
    ...
    for dataset, model, lr in hparams:
        ...
        # savefile += ".emb"
        param = [
            f"data/edges/parsing/ewt/ewt_dev/{dataset}",
            '--dim', str(model['dim']),
            '--hyp', str(model['hyp']),
            '--edim', str(model['edim']),
            '--euc', str(model['euc']),
...
def run(run_name, gpus=1, gpc=1, epochs=50, batch_size=65536):
